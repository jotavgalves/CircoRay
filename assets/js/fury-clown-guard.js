...
function ensureStyles(){
  ...
  style.textContent = `
    ...
    #clownFuryImg{display:none;position:absolute;left:0;bottom:0;width:100%;height:auto;z-index:21;pointer-events:auto;cursor:pointer;touch-action:manipulation;user-select:none;-webkit-user-drag:none;object-fit:contain;object-position:center bottom;transform:none!important;transform-origin:center bottom;filter:drop-shadow(0 0 18px #ff1d1d) drop-shadow(0 0 42px #b00000) drop-shadow(0 0 76px #5a0000)}
...
function triggerClownSpeech(main, event){
  if (typeof window.CIRCO_CLOWN_TAP === "function") {
    window.CIRCO_CLOWN_TAP();
    return;
  }
  try {
    main.dispatchEvent(new PointerEvent("pointerup", {
      bubbles:true,cancelable:true,pointerType:event?.pointerType||"mouse",clientX:event?.clientX||0,clientY:event?.clientY||0
    }));
  } catch {
...
function proxyInteraction(fury, main){
  ...
  fury.addEventListener("pointerup", event => {
    ...
    triggerClownSpeech(main,event);
  });

  fury.addEventListener("keydown", event => {
    if (!shouldBeFury() || (event.key !== "Enter" && event.key !== " ")) return;
    event.preventDefault();
    if (typeof window.CIRCO_CLOWN_TAP === "function") window.CIRCO_CLOWN_TAP();
    else main.dispatchEvent(new KeyboardEvent("keydown",{key:event.key,bubbles:true,cancelable:true}));
  });
}

function ensureFuryImage(){
  const wrap=getWrap();const main=getMainClown();if(!wrap||!main)return null;
  let fury=document.getElementById("clownFuryImg");
  if(!fury){fury=document.createElement("img");fury.id="clownFuryImg";fury.alt="Palhaço em modo Fúria";fury.draggable=false;wrap.appendChild(fury)}
...
function applyFury(){
  if(!shouldBeFury())return;furyActive=true;document.body.classList.add("cr-hardcore-fury");ensureStyles();const fury=ensureFuryImage();
  if(fury){fury.style.display="block";fury.style.visibility="visible";fury.style.opacity="1";fury.style.pointerEvents="auto"}
}

function bindMain(){
  const main=getMainClown();if(!main||mainObserver)return;
  mainObserver=new MutationObserver(()=>{if(shouldBeFury())applyFury()});
  mainObserver.observe(main,{attributes:true,attributeFilter:["src","class","style"]});
}

function activate(){furyActive=true;ensureStyles();bindMain();applyFury();requestAnimationFrame(applyFury);setTimeout(applyFury,50);setTimeout(applyFury,180)}
function reset(){furyActive=false;const fury=document.getElementById("clownFuryImg");if(fury)fury.style.display="none"}

export function initFuryClownGuard(){
  if(window.__circorayFuryClownGuardV4Loaded)return;
  window.__circorayFuryClownGuardV4Loaded=true;
  ensureStyles();bindMain();ensureFuryImage();
