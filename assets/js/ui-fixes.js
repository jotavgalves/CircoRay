export function initUiFixes(){
  ...
  style.textContent=`
    ...
    /* Palhaço: raiva muda luz, nunca a posição corporal. */
    ...
    /* Ranking só existe visualmente quando a partida terminou. */
    ...
    #cr-ranking-btn.cr-ranking-final-visible{display:block!important}
  ...
  function isFinalScreen(){
    return Boolean(document.querySelector('#screen-coupon.active,#screen-win.active,#screen-lose.active,#screen-closed.active,.screen.active[id*="coupon"],.screen.active[id*="win"],.screen.active[id*="lose"],.screen.active[id*="closed"]'));
  }

  function syncRanking(){
    const btn=document.getElementById('cr-ranking-btn');
    if(!btn)return;
    btn.classList.toggle('cr-ranking-final-visible',isFinalScreen());
  }
