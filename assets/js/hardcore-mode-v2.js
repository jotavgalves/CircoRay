...
function installStyles() {
  ...
  .crhc-wall-danger{animation:crhcWallDanger .42s ease-in-out infinite alternate!important}
  .crhc-wall-danger .crhc-player{box-shadow:0 0 12px #fff,0 0 26px #ff1d1d!important}
  @keyframes crhcDanger{to{box-shadow:inset 0 0 70px rgba(255,0,0,.35)}}
  @keyframes crhcWallDanger{from{box-shadow:inset 0 0 35px rgba(160,0,0,.42)}to{box-shadow:inset 0 0 105px rgba(255,0,0,.72)}}
...
function startChase() {
  ...
  const desktopDifficulty = desktop ? 1.18 : 1;
  const baseSpeed = (fury ? state.cfg.chaseFurySpeed : state.cfg.chaseClownSpeed) * 0.12 * desktopDifficulty;
  ...
  function blocked(x, y) { return walls.some((wall) => circleHitsRect(x, y, PLAYER_RADIUS, wall)); }
  function touchingOuterWall() {
    const eps = 1.25;
    return px <= PLAYER_RADIUS + eps || py <= PLAYER_RADIUS + eps || px >= arena.clientWidth - PLAYER_RADIUS - eps || py >= arena.clientHeight - PLAYER_RADIUS - eps;
  }
  function position() {
    player.style.left = `${px}px`; player.style.top = `${py}px`;
    chaser.style.left = `${cx}px`; chaser.style.top = `${cy}px`;
  }
  function movePlayer(clientX, clientY) {
    const r = arena.getBoundingClientRect();
    let targetX = clamp(clientX - r.left, PLAYER_RADIUS, r.width - PLAYER_RADIUS);
    let targetY = clamp(clientY - r.top, PLAYER_RADIUS, r.height - PLAYER_RADIUS);
    const playerMultiplier = fury && wallPenalty ? 0.75 : 1;
    targetX = px + (targetX - px) * playerMultiplier;
    targetY = py + (targetY - py) * playerMultiplier;

    const totalDx = targetX - px, totalDy = targetY - py;
    ...
    for (let i = 0; i < steps; i++) {
      const nextX = clamp(px + stepX, PLAYER_RADIUS, r.width - PLAYER_RADIUS);
      const nextY = clamp(py + stepY, PLAYER_RADIUS, r.height - PLAYER_RADIUS);
      let moved = false;
      if (!blocked(nextX, py)) { px = nextX; moved = true; }
      if (!blocked(px, nextY)) { py = nextY; moved = true; }
      if (!moved) break;
    }
    position();
  }
  ...
  function updateWallPenalty(now, dt) {
    if (!fury) return 1;
    if (touchingOuterWall()) {
      wallTouchMs += dt;
      if (wallTouchMs >= 350 && !wallPenalty) {
        wallPenalty = true;
        arena.classList.add("crhc-wall-danger");
        if (now >= nextWallTauntAt) {
          say(Math.random() < .5 ? "SAIA DA PAREDE." : "AS PAREDES NÃO VÃO TE SALVAR.", 1700);
          nextWallTauntAt = now + 3200;
        }
      }
      if (wallTouchMs >= 1500 && now >= wallBurstUntil) {
        wallBurstUntil = now + 900;
        wallTouchMs = 700;
        if (now >= nextWallTauntAt) {
          say("EU CONSIGO TE VER.", 1500);
          nextWallTauntAt = now + 3200;
        }
      }
    } else {
      wallTouchMs = Math.max(0, wallTouchMs - dt * 2);
      if (wallTouchMs === 0 && wallPenalty) {
        wallPenalty = false;
        arena.classList.remove("crhc-wall-danger");
      }
    }
    if (now < wallBurstUntil) return 1.35;
    return wallPenalty ? 1.20 : 1;
  }

  function loop(now) {
    if (dead) return;
    const dt = Math.min(32, Math.max(0, now - last)); last = now;
    const wallClownMultiplier = updateWallPenalty(now, dt);
    if (now >= nextDashAt) {
    ...
    const speed = baseSpeed * acceleration * dashMultiplier * wallClownMultiplier;
    ...
    if (dist < 48) {
      dead = true;
      arena.classList.remove("crhc-wall-danger");
      state.memory.losses += 1; state.memory.lastResult = "hardcore-chase-lose"; addAnger(6, "loss"); saveMemory();
      showRetry("ELE TE PEGOU.", "Você não atravessa paredes. Ele atravessa.", startChase); return;
    }
    if (remaining <= 0) { dead = true; arena.classList.remove("crhc-wall-danger"); fillExtra(0); startDefend(); return; }
